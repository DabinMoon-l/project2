...
export default function AIQuizPlayer({
  ...
  if (showResult) {
    ...
    return createPortal(
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#F5F0E8]" style={{ left: 'var(--modal-left, 0px)', right: 'var(--modal-right, 0px)' }}>
        <motion.div
        ...
        >
          ...
          {/* 점수 */}
          <h2 className="text-5xl font-black text-[#1A1A1A] mb-1">
            {correctCount}<span className="text-2xl">/{questions.length}</span>
          </h2>
          ...
          <button
            ...
            className="w-full py-3 font-bold text-sm border-2 border-[#1A1A1A] bg-[#1A1A1A] text-white rounded-lg shadow-[3px_3px_0px_#1A1A1A] active:shadow-none active:translate-x-[3px] active:translate-y-[3px] transition-all"
          >
            학습 완료
          </button>
        </motion.div>
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex flex-col bg-[#F5F0E8]" style={{ left: 'var(--modal-left, 0px)', right: 'var(--modal-right, 0px)' }}>
      {/* 헤더 */}
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <AnimatePresence mode="wait">
          ...
          >
            {/* 문제 */}
            <div className="p-3 border-2 border-[#1A1A1A] bg-white rounded-lg">
              <div className="flex items-start gap-2">
                <span className="flex-shrink-0 w-7 h-7 flex items-center justify-center border-2 border-[#1A1A1A] bg-[#1A1A1A] text-white font-bold text-xs rounded-md">
                  Q
                </span>
                <div className="flex-1">
                  <p className="text-[#1A1A1A] text-sm font-medium leading-relaxed pt-1">
              ...
              {currentQuestion.choices.map((choice, idx) => {
                ...
                return (
                  <button
                    ...
                    disabled={isAnswered}
                    className={`w-full p-3 border-2 text-left text-sm rounded-lg transition-all ${buttonStyle}`}
                  >
                    <div className="flex items-start gap-2">
            ...
            {isMultipleAnswer && !isAnswered && (
              <button
                ...
                className={`w-full py-2.5 font-bold border-2 rounded-lg transition-all ${
            ...
            <AnimatePresence>
              {isAnswered && currentQuestion.explanation && (
                <motion.div
                  ...
                  className="p-3 border-2 border-[#1A1A1A] bg-[#EDEAE4] rounded-lg"
                >
                  <div className="flex items-start gap-2">
