...
import { useDetailPanel, useDetailPosition } from '@/lib/contexts';
...
export default function FolderSelectModal({
  ...
  const content = (
    <>
      {/* 헤더 */}
      <div className="p-3 border-b border-[#1A1A1A] flex items-center justify-between flex-shrink-0">
      ...
      {/* 새 폴더 만들기 */}
      <div className="p-3 border-b border-[#D4CFC4] flex gap-2 flex-shrink-0">
        <input
          type="text"
          value={newFolderName}
          ...
          placeholder="새 폴더 이름"
          className="flex-1 min-w-0 px-3 py-2 text-xs border border-[#1A1A1A] bg-[#FDFBF7] text-[#1A1A1A] placeholder-[#5C5C5C] focus:outline-none rounded-lg"
        />
        <button
          onClick={handleCreate}
          disabled={!newFolderName.trim() || creating}
          className="px-3 py-2 text-xs font-bold bg-[#1A1A1A] text-[#F5F0E8] hover:bg-[#333] disabled:opacity-40 transition-colors rounded-lg whitespace-nowrap flex-shrink-0"
        >
          {creating ? '...' : '생성'}
        </button>
      </div>
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute bottom-0 left-0 right-0 z-[105] bg-[#F5F0E8] border-t-2 border-[#1A1A1A] rounded-t-2xl overflow-hidden flex flex-col max-h-[70%]"
            >
              {/* 핸들 */}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          style={{ left: 'var(--modal-left, 0px)', right: 'var(--modal-right, 0px)', paddingBottom: 'var(--kb-offset, 0px)' }}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          onClick={onClose}
