...
function ExpToastContainer({ toasts }: { toasts: ExpToastData[] }) {
  return (
    <div className="fixed left-0 right-0 z-[120] pointer-events-none flex flex-col items-center gap-2 px-4" style={{ top: 'calc(4.5rem + env(safe-area-inset-top, 0px))', left: 'var(--modal-left, 0px)', right: 'var(--modal-right, 0px)' }}>
      <AnimatePresence>
        {toasts.map((toast) => (
