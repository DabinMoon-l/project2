...
**⚠️ 교수 서재에서 학생 `custom` 퀴즈 수정 시 `type`이 `midterm`으로 덮어쓰이는 버그 방지됨** — 원본 type이 교수 퀴즈 타입이 아니면 type 변경 차단
**⚠️ 교수 자작탭의 커스텀 퀴즈 쿼리에 `isPublic: true` 필터 적용** — 학생 비공개 퀴즈 노출 방지
**⚠️ 교수 미리보기에서 본인 퀴즈가 아니면 삭제/수정 버튼 숨김** (`isOwner` 체크)
...
문제 텍스트, 선지, 제시문, 보기, 해설에서 인라인 서식을 지원:
- `*이탤릭*` → *이탤릭* (학명 등)
- `{아래첨자}` → 아래첨자 (CO{2} → CO₂)
- `^위첨자^` → 위첨자 (m^2^ → m²)

적용 위치: `MixedExamplesRenderer`, `MultipleChoice`, `QuestionCard`(퀴즈/복습/피드백/결과), `PreviewQuestionCard`, `CombinedQuestionGroup`, `UpdateQuizModal`, `ReviewQuestionCard`, `PracticeStage`, `ResultStage`

### 제시문/보기 저장 필드

퀴즈 생성(`/quiz/create`) 및 수정(`EditQuizSheet`) 시 반드시 포함해야 하는 필드:
- `bogi` — 보기 데이터 (ㄱ.ㄴ.ㄷ. 형식)
- `passagePrompt` — 제시문 발문
- `passageBlocks` — 제시문 블록 (텍스트/가나다/불렛/이미지/그룹)
- `mixedExamples` — 혼합 보기 (text/labeled/gana/bullet/image/grouped)

**⚠️ `gana`, `bullet` 타입은 저장 필터에서 누락되기 쉬움** — switch문의 default에 빠지지 않도록 주의
...
| 퀴즈 수정 후 제시문/보기 사라짐 | 저장 시 bogi/passageBlocks 누락 | 저장 객체에 필드 포함 확인 |
| 퀴즈 수정 후 type이 midterm으로 변경 | editType 기본값이 midterm | originalType 체크로 방어 |
| 재채점 시 객관식 점수 오류 | restoreUserAnswer에서 -1 적용 | 0-indexed 그대로 파싱 (수정됨) |
| 피드백 OX 오답 표시 | userAnswer 정규화 누락 | "0"→"O", "1"→"X" 변환 추가 |
