...
**RabbiTory** — 대학 수업 보조 PWA (147,000줄+, 666커밋). AI 기반 퀴즈 생성 + 게시판 + 토끼 컨셉 게이미피케이션.
...
| 항목 | 수치 |
|------|------|
| 프론트엔드 (TSX+TS) | 122,162줄, 411파일 |
| Cloud Functions (TS) | 25,395줄, 70파일 |
| **총 코드** | **147,557줄** |
| 컴포넌트 (TSX) | 257개 |
...
- **Firebase** — Auth, Firestore, RTDB(배틀 전용), Cloud Functions(Node 22), Storage, FCM
...
- `lib/repositories/` — Firestore/RTDB/Storage 추상화 (12개 도메인 repo)
...
**수정 문제 다시 풀기**: 서재탭에서 교수가 수정한 문제에 골든 뱃지(!) 표시, 클릭 시 `practiceOnly` 모드로 점수 변동 없이 복습
...
**문제 풀 (사전 생성)**:
- Cloud Run `tekkenPoolRefillScheduled` — 매월 1일 03:00 KST `0 3 1 * *` (비용 절감, 기존 매일)
...
- 동점 시 같은 순위 (1위, 1위, 3위)
- 일간/주간/전체 필터
- **순위 변동**: `prevDayRanks` 일간 스냅샷 — 00:00 KST 기준 전일 랭킹 저장, 같은 날 내 유지
...
상세 설계: `docs/saas-architecture.md` (1,166줄, 28 테이블 스키마 + 7단계 마이그레이션)

### 타겟 스택

Firebase Auth → **Better Auth** (Organization 플러그인, SSO) / Firestore → **Supabase PostgreSQL** (RLS 테넌트 격리) / CF → **Supabase Edge Functions + Cloud Run** (AI) / 배틀 RTDB + FCM은 Firebase 유지

### Phase 0: orgId 주입 (2주, LOW)
- 기존 코드에 `orgId` 필드 추가, 단일 대학 하드코딩 → 추후 동적 전환

### Phase 1: Better Auth + PostgreSQL 스키마 (4주, MEDIUM)
- `organizations` 테이블 (대학별 slug/logo/theme) + `org_members` (role, student_id, class_id)
- JWT claims에 `org_id`, `org_role` 포함

### Phase 2: Firestore → PostgreSQL 마이그레이션 (6주, HIGH)
- Repository 패턴 활용 (`lib/repositories/` import 교체만으로 전환)
- Firestore onSnapshot → Supabase Realtime
- 21개 훅 마이그레이션

### Phase 3: CF → Edge Functions (4주, MEDIUM)
- Cloud Functions → Supabase Edge Functions + Cloud Run (AI)
- Vercel 유지 (프론트)

### Phase 4: RTDB + Storage 마이그레이션 (3주, MEDIUM)
- 배틀은 Firebase RTDB 유지 (50ms 지연 요구)
- Firebase Storage → Supabase Storage

### Phase 5: 동적 Scope/FocusGuide (3주, MEDIUM)
- 교수가 PDF/PPTX 업로드 → AI가 챕터 + Scope + Focus Guide 자동 추출
- `course_focus_guides` 테이블 (JSONB, priority 태그)
- 현재 하드코딩(`styledQuizGenerator.ts`) → DB 동적 관리 + 교수 편집 UI

### Phase 6: 셀프 온보딩 + 빌링 (3주, LOW)
- 교수 셀프 가입 → 과목 생성 → 초대 코드로 학생 등록
- Stripe/Paddle 결제 연동

### Phase 7: App Store 출시 (2주, LOW)
- 글로벌 영어권 대학 타겟 (미국 4,000+ 대학)
- `next-intl` i18n, 영어 UI + AI 프롬프트 로컬라이제이션

### Phase 2 병행: 컴포넌트 리팩토링

Supabase 데이터 마이그레이션 시 해당 파일을 건드릴 때 병행. useState 과다 사용 컴포넌트 정리.

| 파일 | useState | 리팩토링 패턴 |
|------|---------|-------------|
| `review/[type]/[id]/page.tsx` | 37 | 모드 플래그 6개 → useReducer, 편집 상태 → useEditQuiz 훅, data+loading 쌍 통합 |
| `professor/quiz/create/page.tsx` | 37 | OCR/문서/이미지 상태 → 커스텀 훅 3개 추출 |
| `AnnouncementChannel.tsx` | 35 | 검색→useSearch, 투표→usePollEditor, 미디어→useMediaUpload, 캘린더→useCalendar |
| `quiz/create/page.tsx` | 30 | OCR→useOCR, 문서→useDocumentUpload, 이미지→useImageExtraction |
...
|------|----------------|-------------------|
| **랭킹 계산** | 2시간 | **10분** (`pg_cron` + MV) |
| **레이더 정규화** | 2시간 | **10분** (`pg_cron` + MV) |
| **배틀 문제 풀** | 매월 1일 | **매일** |
| **쿼리** | `.select()` 필드 제한 | **전체 row** (읽기 무제한) |
| **주간 통계** | 미구현 (비용 과다) | **SQL GROUP BY** |

**핵심 이점**: Firestore 문서 읽기 과금 → PostgreSQL 쿼리 단위 실행, 복잡한 집계도 단일 쿼리.
...
| 파일 | 줄 수 | 내용 |
|------|-------|------|
| `review/[type]/[id]/page.tsx` | 2,145 | 폴더 상세 + 복습 플로우 |
| `quiz/create/page.tsx` | 1,989 | 퀴즈 만들기 6종 편집기 |
| `styledQuizGenerator.ts` | 1,910 | AI 프롬프트 10레이어 조합 |
| `announcement/AnnouncementChannel.tsx` | 1,879 | 공지 채널 |
| `professor/quiz/create/page.tsx` | 1,711 | 교수 퀴즈 출제 |
| `lib/ocr.ts` | 1,692 | Clova OCR 파이프라인 |
| `review/page.tsx` | 1,638 | 복습 5탭 + 연습 |
| `professor/quiz/[id]/preview/page.tsx` | 1,632 | 교수 미리보기 |
| `quiz/create/QuestionEditor.tsx` | 1,607 | 문제 편집기 UI |
| `QuizStatsModal.tsx` | 1,551 | 퀴즈 통계 모달 |
