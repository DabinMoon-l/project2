...
2. 교수 홈 — 과목 선택 + 공지 작성 + 학생 활동 모니터링
   ↓
3. 퀴즈 관리
   ├── 직접 출제: /professor/quiz/create → 6종 문제타입 + 이미지 + 서술형 루브릭
   ├── AI 서재: 프롬프트+난이도+챕터 → 백그라운드 생성 → 편집 → 공개 전환
   └── 미리보기: 문제별 정답률/변별도 통계 + 재채점
   ↓
4. 학생 분석
   ├── 6축 레이더 차트 (석차/성장/출제/소통/복습/활동)
...
- **난이도**: easy (기억/이해) / medium (적용/분석) / hard (분석/평가+함정+복수정답+보기형)
- **문제 수**: 5~20개
- **챕터 태그**: 과목별 동적 태그 필수 선택
- **교수 프롬프트**: 최우선 출제 지시사항 (교수 서재에서만)
...
- **보기형 문제**: `bogi: { questionText, items: [{label: "ㄱ", content: "..."}] }`
- **복수정답**: `answer: [0, 2]` 배열
- **부정형**: "옳지 않은 것" 30% 비율
...
```
ProfessorLibraryTab → 프롬프트+난이도+태그 입력
  → enqueueGenerationJob (백그라운드)
  → 다른 페이지 이동해도 생성 계속 (LibraryJobManager 이벤트 구독)
  → 완료 시 LibraryJobToast 표시 (어떤 페이지에서든)
  → 서재 카드 → Details/Preview/편집/공개전환/삭제
...
AIQuizContainer 플로팅 버튼 → AIQuizModal (PDF/PPTX/이미지 업로드)
  → 태그+난이도+문제수 선택 → enqueueGenerationJob
...
**플로우**: 매칭(30초, 봇 폴백) → countdown → question(20초) → 양쪽정답시 mash(연타) → roundResult → ... → finished(KO or 소진)
...
**봇**: 40% 정답률, 1~8초 응답, 10개 닉네임 풀, 레벨 3~7
...
### 6축 레이더 차트 (10분마다 사전 계산)

| 축 | 이름 | 계산 | 스케일 |
|----|------|------|--------|
| 1 | 가중 석차 | 교수 퀴즈 석차 가중 평균 (참여 <5명: 원점수, ≥5명: 백분위) | 0~100 |
| 2 | 성장세 | 재시도 개선율 | 0~100 |
| 3 | 출제력 | 커스텀 퀴즈 수 | 백분위 |
| 4 | 소통 | 게시글×3 + 피드백 | 백분위 |
| 5 | 복습력 | reviewCount 합계 | 백분위 |
| 6 | 활동량 | totalExp | 백분위 |
...
| computeRadarNormScheduled | 10분 | 6축 레이더 |
