...
**검정(단순·표준, 전 변수 동일)**:
- **비모수 Jonckheere–Terpstra 순서대립 추세검정**(양측·비방향) + **효과크기(Cliff's δ)·95%CI**. **R 검증 패키지**(`DescTools`·`effsize`·`boot`), 직접 구현·정식 ANCOVA·다자유도 옴니버스 없음.
- 군집 코딩 `grade_cluster`: **상=+1, 중=0, 하=−1** (순서).
- 공변량(노출기간·분반) 군집 간 불균형은 *기술·한계 명시*(필요 시 단순 층화 기술 점검) — 정식 보정·"주·보조 불일치=교란" 규칙 없음.
- 다중비교·p: 확증 주가설 없는 기술형 → 단일 α 미의존. **본문 효과크기·95%CI만**, 변수별 p 미보고. p는 부록 표 "k검정·무보정·비추론·확증 아님" 라벨로만.
- `feedback_count` 등 0편중: 허들·음이항 *모형* 대신 **2부 기술**(① 1건+ 제출자 비율+95%CI ② 제출자 중앙값 IQR).

**보고 기조**: **본문은 효과크기 + 95% CI만**, 변수별 p 미보고(p는 부록 라벨표). 표본 근거는 검정력이 아니라 **정밀도(추정의 한계)**로 제시:
- N≈148, 군집 셀 ≈49/50/49. 이 규모에서 군집 간 *효과크기 f≈0.23 미만의 차이는 95% CI로 안정적으로 구분되지 않을* 수준(정밀도 한계). 참조 수치: G*Power Sensitivity(3군집·공변량 보정 기준, α=0.05, 1-β=.80) f≈0.23 — IRB 표본근거 칸용, 검정력 주장이 아니라 *구분 가능 최소차이*(실제 분석은 J-T+효과크기·CI). 폐기된 옴니버스(α=0.0167) 미사용.
