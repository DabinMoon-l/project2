...
- **표준 절차(전 변수 동일·단순)**: 성적 군집을 순서(상→중→하)로 두고 **비모수 Jonckheere–Terpstra 순서대립 추세검정**(양측·비방향) + **효과크기(Cliff's δ 등)·95% CI**를 산출. **R 검증 패키지**(`DescTools`·`effsize`·`boot`)로, 직접 구현·정식 공분산분석·다자유도 옴니버스 **없음**. 근거: `distribution-priors.md`.
- 공변량(`exposure_days`,`class_id`)의 군집 간 불균형은 *기술·한계 명시*(필요 시 단순 층화 기술 점검) — 정식 ANCOVA 보정·"주·보조 불일치=교란" 규칙 **두지 않음**(과적합·해석 부담 회피).
- `feedback_count` 등 0 편중 변수: 모형(허들·음이항) 대신 **2부 기술** — ① 군집별 1건+ 제출자 비율(비율+95%CI) ② 제출자 중 중앙값(IQR).
...
- 표본 근거 = **정밀도**(검정력 아님): N≈148, 군집 셀 ≈49/50/49 → 효과크기 **f≈0.23 미만 군집차이는 95%CI로 안정 구분 불가**. 참조 수치 G*Power Sensitivity(3군집·공변량 보정 기준, α=0.05·1-β=.80) f≈0.23 = *IRB 표본근거 칸용·구분 가능 최소차이*(실제 분석은 J-T+효과크기·CI). 폐기된 옴니버스(α=0.0167) 미사용.
- 실데이터≠사전가정이어도 표준 절차 불변, 단순 민감도 기술만 추가(`distribution-priors.md` 사전 고정).
