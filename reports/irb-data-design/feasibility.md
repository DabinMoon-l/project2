...
- 분석: **비모수 Jonckheere–Terpstra 순서대립 사전 주분석**(영과잉·유계 다수), 순위변환 ANCOVA + 2-df 옴니버스 보조. 군집 선형코딩(상=+1·중=0·하=−1), 공변량 `exposure_days`,`class_id`.
...
- 실데이터가 사전 가정과 달라도 **주분석(J–T) 불변**, 민감도 분석(허들·변환회귀)만 보조 추가 → 9.3 사전고정과 정합.
...
`feedback_count`는 노력 드는 선택적 행동 → **0건 학생 다발(영과잉·우편향) 가능성 큼**. IRB 전이라 실분포 미상이나 설계상 대비:
- 정규 ANCOVA 가정 위배 가능 → **1-df 추세는 비모수(Jonckheere–Terpstra, 순서 대립) 또는 음이항/영과잉 회귀** 병행.
- "제출 여부(≥1건, 이분)"와 "제출자 내 강도"를 분리(허들 구조)해 0과잉 흡수.
- 어떤 경우든 **효과크기(Cliff's δ 등)+95%CI만 본문 보고**, p는 부록 라벨표로만. 종강 후 실분포 확인하여 모형 확정.
