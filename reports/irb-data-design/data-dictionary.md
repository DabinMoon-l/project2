...
> **분석(단순·표준)**: 변수별 **비모수 Jonckheere–Terpstra 순서대립 추세검정**(양측·비방향) + **효과크기(Cliff's δ)·95%CI**, **R 패키지**(DescTools·effsize·boot)로. 정식 ANCOVA·다자유도 옴니버스·교란해석 규칙 없음. 공변량(exposure_days·class_id) 불균형은 기술·한계로만. `feedback_count` 등 0편중=**2부 기술**(1건+ 제출비율 / 제출자 중앙값). 실데이터≠사전가정이어도 표준 절차 불변·단순 민감도 기술만.
> **다중비교·p**: 확증 주가설 없는 기술형 → 단일 α 미의존. **본문은 효과크기+95%CI만**, 변수별 p 미보고. p는 부록 표 *"k검정·무보정·비추론·확증 아님"* 라벨로만 일괄 제시.
> **표본 근거(정밀도)**: N≈148, 군집 셀 ≈49/50/49. 이 규모에서 효과크기 **f≈0.23 미만의 군집 차이는 95% CI로 안정적으로 구분되지 않음**(정밀도 한계). 참조 수치 G*Power Sensitivity(3군집·공변량 보정 기준, α=0.05, 1-β=.80) f≈0.23 — IRB 표본근거 칸용, 검정력 주장 아닌 *구분 가능 최소차이*(실제 분석은 J-T+효과크기·CI). 폐기된 옴니버스(α=0.0167) 미사용.
