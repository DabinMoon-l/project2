...
`;
  ...
  // 미생물학 — 교수 출제 스타일 기반 병원체 중심 (단, 어디까지나 Scope 범위 안에서)
  if (courseName.includes("미생물")) {
    prompt += `\n[미생물학 출제 방향 — 교수님 출제 스타일 반영]
미생물학은 병원체를 다루는 과목입니다. 아래 발문 스타일·출제 포인트를 따르되 **반드시 위 Scope에 서술된 병원체·질환 안에서만** 출제하세요. (배틀은 30초 제한 → 발문은 짧고 직접적으로)

▶ 발문 스타일 (짧고 빠른 직접 매핑 위주):
- 질환 → 병원체: "○○를 일으키는 병원체는?"
- 병원체 → 질환/소견: "○○ 바이러스가 일으키는 질병은?"
- 특징적 소견·별명 → 병원체: "올빼미눈 봉입체를 만드는 바이러스는?", "아구창의 원인 미생물은?"
- 분류 소속: "DNA 바이러스에 속하는 것은?", "외피 보유 바이러스는?" (영어 학명도 선지로 활용)
- 전파경로·매개체: "대변-구강 경로로 전파되는 것은?", "모기 매개 질환은?"

▶ 핵심 출제 포인트 (★같은 병원체도 여러 각도로 → 반복 배틀에서 다양한 문제가 나오게):
1. 병원체 ↔ 질환 ↔ 증상/특징적 소견 (가장 중요)
2. 전파 경로·매개체·감염원
3. 분류 (DNA/RNA·외피 유무·과(family) 소속)
4. 백신·항미생물제 작용 (Scope에 있으면)
5. 종양·만성화 연관 (예: 간염→간암 — Scope에 있으면)
6. 법정감염병 급수·인수공통감염 (Scope에 있으면)

▶ 다양성: 한 병원체를 질환/증상/전파/분류/치료 등 **서로 다른 각도**로 출제해 학생이 반복 배틀로 폭넓게 복습하게 하세요.
⚠️ 위는 "무엇을 묻는지" 안내일 뿐 — 내용·정답은 전부 Scope에서 확인. Scope에 없는 병원체·질환·약물은 절대 출제 금지.
`;
  }
...
## ⛔ 금지 문제 유형 (반드시 피할 것)
- "~은 무엇인가?", "~의 정의는?", "~을 담당하는 것은?", "~에 해당하는 것은?" 같은 단순 정의/용어/나열 문제
- 한 단어만 알면 풀리는 단순 암기 문제
- "다음 중 올바르지 않은 것은?" 같은 소거법 유도 문제
- 대신 **비교·구별·적용·인과관계·조건 판단·상황 추론** 중심으로 출제

## ✅ 좋은 문제 유형 (이런 식으로 출제)
- 비교: "A와 B의 차이로 올바른 것은?"
- 상황 추론: "~상황에서 나타나는 반응은?", "면역력 저하 환자에게 가장 위험한 감염은?"
- 인과관계: "~가 억제되면 어떤 결과가 나타나는가?"
- 조건 판단: "~조건에서 활성화되는 면역세포는?"
- 임상 적용: "간호사가 ~환자를 관찰할 때 주의해야 할 것은?"
...
JSON 배열로 출력: [{"text":"문제","type":"multiple","choices":["선지1","선지2","선지3","선지4"],"correctAnswer":0,"difficulty":"${difficulty}","explanation":"정답 해설","choiceExplanations":["선지1 해설","선지2 해설","선지3 해설","선지4 해설"],"chapterId":"3"}]`
      ...
      // Flash thinking OFF (프롬프트 강화로 퀄리티 확보, 2주 1회로 비용 절감)
