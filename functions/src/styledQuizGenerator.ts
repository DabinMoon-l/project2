...
- **(필수 출제) 선천면역 vs 후천면역(적응면역) 비교**:
  - 반응 속도: 선천=즉각(수시간) vs 후천=느림(수일), 단 2차 반응은 빠름
  - 특이성: 선천=비특이적(광범위) vs 후천=항원 특이적
  - 면역기억: 선천=없음 vs 후천=있음(기억B/T세포 → 2차 반응 강화)
...
- **(필수 출제) 후천면역의 면역기억**: 1차 면역반응 vs 2차 면역반응, 기억B세포·기억T세포의 역할, 2차 반응의 신속성·강도 증가, 백신의 원리(면역기억 유도)
- **(필수 출제) 보체 시스템**: 활성화 경로(고전경로, 대체경로, 렉틴경로), C3b=옵소닌화, 아나필라톡신(C3a/C5a — 비만세포 탈과립·염증 유발), C5b→MAC(막공격복합체)
- **(필수 출제) 항체(면역글로불린) 구조와 종류 — IgG·IgM 중심, IgA 참고**:
...
- **(필수 출제) 1차/2차 항원 노출과 항체 반응 (그래프)**:
  - 1차 노출: IgM 먼저 출현 → 4~5일 후 IgG 상승, 전체 반응 느림
  - 2차 노출: IgM·IgG 거의 동시 출현, 급격한 항체 농도 상승 → 면역반응 현저히 빠름
  - 핵심: 1차=IgM 우세, 2차=IgG 우세, 기억B세포의 신속 분화가 2차 대응 속도 결정
- **(필수 출제) 면역세포 분화·기능 비교**: 보조T세포(Th) vs 세포독성T세포(CTL) vs 형질B세포(항체 분비) vs 기억B세포(2차 반응 대비) — 기억면역과 연계 출제
- **(필수 출제) Fc 수용체와 옵소닌화**: 대식세포가 항체 Fc 부분을 Fc수용체로 인식 → 탐식 촉진, 선천면역-후천면역 연결고리
- **(필수 출제) 사이토카인과 인터페론**:
  - 제1형 인터페론(IFN-α/β): 바이러스 감염세포가 분비 → 주변세포 항바이러스 상태 유도 (선천면역)
  - 제2형 인터페론(IFN-γ): Th1·NK세포 분비 → 대식세포 활성화 (후천면역 연계)
- **(필수 출제) 능동면역 vs 수동면역 + 자연/인공 분류 (2×2 표)**:
  - 자연능동면역: 감염 후 스스로 항체·면역세포 생성 → 면역기억 형성 (오래 지속)
  - 인공능동면역: **백신** 접종 → 약독화/불활성화 항원으로 면역반응 유도 → 면역기억 형성
  - 자연수동면역: 태반 통한 모체 IgG 전달, 모유 통한 IgA 전달 → 신생아 일시적 보호
  - 인공수동면역: **항혈청·면역글로불린** 주사 → 이미 만들어진 항체 직접 투여 (즉각 효과, 단기간)
  - 핵심 구분: 능동=자기 면역계가 항체 생산(느리지만 기억 형성) vs 수동=외부 항체 수여(즉각이지만 기억 없음, 일시적)
  - 자주 혼동: 백신=인공**능동**(자기가 면역반응), 항혈청=인공**수동**(남이 만든 항체)
- **(고빈도) 백신과 집단면역**: 생백신(약독화) vs 사백신(불활성화), 면역기억 유도 원리, 집단면역 개념
- **(고빈도) 면역세포 활성화 연쇄**: APC 항원제시 → Th세포 활성화 → B세포 분화(형질B/기억B) + CTL 활성화 + 대식세포 활성화
- **(필수 출제) 바이러스 감염 대응 종합**: NK세포(감염세포 직접 살해) + IFN-α/β(항바이러스) + CTL(특이적 살해) + 대식세포(탐식) + 항체(ADCC·중화)
- **(필수 출제) NK세포**: 선천면역 핵심, 바이러스 감염세포·종양세포 MHC 비의존적 살해, IFN-γ 분비로 대식세포 활성화
- **(고빈도) 후천면역결핍증후군(AIDS)**: HIV → CD4⁺ T세포(Th) 파괴 → 면역체계 붕괴 → 기회감염
- 후천면역의 감염반응: 병원체 유형별 — 세포 내 기생세균(CTL), 엔도솜 내 세균(Th1→큰포식세포)
...
- **(필수 출제) 기회감염**: 면역저하숙주에서 발생, 내인감염과의 구분
- **(필수 출제) 균교대감염**: 항생제 장기 사용 → 정상균무리 파괴 → 내성균 과증식, 기회감염의 한 형태
- **(필수 출제) 접촉감염 (수평전파 중 중요)**:
  - 직접접촉감염: 피부 대 피부, 점막 대 점막, 혈액·체액 직접 접촉 (예: 매독, 임질)
  - 간접접촉감염: 오염된 매개물(fomite) 통한 전파 — 매개체 전파에 해당
- **(고빈도) 잠복감염 vs 지속감염**: 잠복감염(병원체 잠복, 재활성화 — 수두→대상포진) vs 지속감염(지속적 증식·배출) — 지속감염은 만성감염으로 이행 가능
- **(고빈도) 감염 유형 비교**: 무증상감염(불현성) vs 지속감염 vs 잠복감염 vs 증상감염(현성)
- **(고빈도) 감염 경과 4단계**: 잠복기 → 전구기 → 발병기(극기) → 회복기
- **(고빈도) 정상균무리(정상세균총)**: 공생 관계, 불균형(dysbiosis)의 의미
- **(고빈도) 비말 vs 비말핵**: 5μm 기준, 비말감염 vs 공기감염 구분
- **(고빈도) 항생제 vs 항균제 차이**: 항생제(미생물 유래 천연물질) vs 항균제(합성 포함 광의), 항생제 내성균·항균제 내성균의 발생 기전
`;
...
**출제 방향(병원체 중심)**: 각 병원체가 **어떤 감염병(질환)을 일으키는지**를 핵심으로, 그 질환의 증상·특징적 소견·전파경로·매개체까지 연결해 출제하세요. 분류(DNA/RNA·외피 유무·과(family)), 병원성 인자(독소·협막 등), 백신·항미생물제 작용, 종양/만성화 연관(예: 간염→간암), 법정감염병·인수공통감염도 핵심입니다(학습 범위에 있을 때).
**발문 스타일**: "○○를 일으키는 병원체는?", "○○가 일으키는 질병은?", "특징적 소견→병원체", "○○과에 속하는 것은?", "전파경로/매개체는?" 등 직접 매핑 위주. 같은 병원체도 질환/증상/전파/분류/치료 등 여러 각도로 출제해 폭넓게 학습하게 하세요.
...
export const DIFFICULTY_PARAMS = {
  easy: {
   ...
   - ⚠️ "옳은 것은?" 발문 시 정답 1개만 사실이어야 하고 나머지는 명확히 틀려야 함. 여러 선지가 사실상 옳은데 "가장 적절한 것"을 고르는 식의 애매한 문제 절대 금지.`,
  ...
  medium: {
   ...
   - ❌ 금지: 선지가 단어나 짧은 구절 나열인 문제 (예: "호흡기계 분비물 (기침, 재채기)"). 이런 나열형은 EASY 수준임.
   - ❌ 금지: 오답이 해당 카테고리에 전혀 속하지 않아서 바로 소거되는 문제 (예: "탈출 경로가 아닌 것은?" → "면역세포 활동" — 너무 뻔함)
   - ✅ 필수: 선지는 반드시 **완전한 서술문**(1문장 이상)이어야 하며, 개념에 대한 설명/기전/비교를 포함해야 함
   - ✅ 필수: 오답도 해당 주제와 관련된 그럴듯한 서술이어야 함 — 핵심 부분만 미묘하게 틀리게`,
    trapStyle: "유사 용어 혼동, 시간 순서 교란, 기전의 미묘한 오류",
    choiceStyle: `유사 개념이 섞인 **서술형 선지** — 각 선지가 개념을 설명하는 완전한 문장이어야 함. 단어 나열 금지.
   - 비슷하지만 다른 용어, 과정 순서, 기전 연결을 섞어 출제
   - 소거에 사고가 필요한 수준 — 오답도 부분적으로 맞는 내용을 포함
   - ⚠️ "옳은 것은?" 발문 시 정답 1개만 사실이어야 하고 나머지는 명확히 틀려야 함. 여러 선지가 사실상 옳은데 "가장 적절한 것"을 고르는 식의 애매한 문제 절대 금지.`,
  ...
  hard: {
    ...
    cognitiveLevel: `분석/평가 — 심층 이해 확인. 단순 정의, 단순 분류, 단순 비교 문제는 HARD가 아닙니다.
   - ❌ HARD 금지 유형: "A와 B를 구분하는 특징으로 옳지 않은 것은?" (단순 비교), "~요인으로 보기 어려운 것은?" (단순 분류), "~의 정의로 옳은 것은?" (단순 정의)
   - ✅ HARD 적합 유형: 두 개념의 기전을 혼동시키는 선지, 조건이 바뀌면 결과가 달라지는 상황, 여러 개념을 교차 적용해야 하는 문제
   - 발문은 보통처럼 간결하되 선지 5개가 전부 그럴듯해서 소거가 극도로 어려워야 함`,
    trapStyle: "정상비정상 뒤집기, 유사 용어 혼동, 부분적으로 맞는 선지, 미묘한 차이",
    choiceStyle: `**핵심 전략: 매력적인 오답 선지** — 선지 5개 모두 그럴듯하게 구성.
   - 오답이 "부분적으로 맞지만 핵심이 틀린" 선지여야 함 (명백히 틀린 선지 금지)
   - 2~3개 선지가 정답처럼 보여서 소거법이 통하지 않아야 함
   - 정확한 개념 이해 없이는 풀 수 없지만, 지엽적이거나 암기형은 아닌 수준
   - 선지가 한 단어이거나 너무 짧으면 HARD가 아님 — 선지마다 구체적인 서술이 포함되어야 함
...
- 모든 문제를 복수정답으로 만들지 말고, 적절한 경우에만 사용하세요 (약 20% 이하)`;
  ...
  if (params.allowPassage) {
    formatInstructions += `
### 제시문(가나다) 형식
- "옳은 것을 모두 고르시오" 유형에서 **(가)(나)(다)(라)** 항목을 나열할 때, 반드시 제시문(passageBlocks)으로 분리하세요.
- ❌ 잘못된 예: text에 "(가) 내용1\\n(나) 내용2" 포함 → 가나다가 발문 안에 섞임
- ✅ 올바른 예: text는 발문만, passageBlocks에 gana 블록으로 분리
- 제시문은 "passageBlocks" 배열로 표현합니다. **3가지 블록 유형**:
  1. **gana** ((가)(나)(다)): 라벨링된 항목 나열 — 가장 자주 사용
  2. **text** (텍스트박스): 단락형 지문 (임상 사례, 실험 결과)
  3. **bullet** (◦ 불렛): 항목 나열 (특징 목록, 증상 목록)
- "passagePrompt"에 제시문 발문을 작성하세요 (예: "다음은 감염 경로에 대한 설명이다.")
- 제시문 예시:
  "passagePrompt": "다음 중 옳은 것을 모두 고르시오.",
  "passageBlocks": [
...
- **제시문이 필요 없는 일반 문제에서는 passageBlocks와 passagePrompt를 생략하세요.**
- (가)(나)(다) 항목이 있는 문제에서만 사용하세요. 모든 문제에 붙이지 마세요.`;
  }
...
### ⚠️ 선지 편향 방지 (필수)
정답이 형식만으로 식별되면 안 됩니다. 다음 규칙을 반드시 지키세요:
1. **길이 균일**: 정답 선지와 오답 선지의 길이를 비슷하게 맞추세요. 정답만 유독 길거나 상세하면 안 됩니다.
2. **구조 균일**: 정답이 "A는 ~이고, B는 ~이다" 양면 비교 구조라면, 오답도 같은 양면 비교 구조를 사용하되 내용만 틀리게 하세요.
3. **극단어 분산**: "반드시", "유일한", "항상", "절대", "~만", "~없다" 같은 한정어를 오답에만 집중 배치하지 마세요. 오답에는 "주로", "대부분", "상대적으로" 같은 정도 표현을 사용하여, 지식 없이는 소거할 수 없게 하세요.
4. **정보량 제한**: 각 선지에 담는 팩트는 최대 2개까지. "A이고, B이며, C이다"처럼 3개 이상의 사실을 나열하지 마세요. 선지가 길다고 어려운 것이 아닙니다. 짧고 담백한 선지에서 핵심 1~2개만 비교하게 하세요.
5. **포괄성 금지**: "N가지를 모두 서술한 선지 = 정답" 패턴을 피하세요. 오답에도 동일한 구조를 사용하되 핵심만 틀리게 하세요.
...
4. **복수정답 형식 활용**: 정답이 2개 이상인 문제(answer 배열)로 출제. 단, "옳은 것은?" 발문에 정답이 1개인 문제에서 다른 선지도 사실상 옳은 애매한 문제는 금지
` : "";
...
- **"옳은 것은?" 발문인데 오답 선지 중 사실상 옳은 진술이 포함된 문제** — 오답은 반드시 학술적으로 틀린 내용이어야 함. 여러 선지가 옳은데 "가장 적절한 것"을 고르는 애매한 문제는 학습에 혼란을 주므로 절대 금지
- 불확실하거나 추측에 기반한 문제
- 최우선 지시사항이 있는데 해당 주제와 무관한 문제
- **HARD 난이도인데 선지가 한 단어이거나, 단순 정의/분류/나열만 묻는 문제** — HARD는 개념 간 교차 적용, 기전 비교, 조건부 판단이 필요해야 함
- **MEDIUM/HARD 난이도인데 선지가 단어 나열이거나 괄호 예시만 붙인 짧은 구절인 문제** — 선지는 반드시 개념을 서술하는 완전한 문장이어야 함
- **오답이 해당 주제와 전혀 무관하여 즉시 소거 가능한 문제** — 오답도 해당 주제 범위 내에서 그럴듯하지만 핵심이 틀린 서술이어야 함
- **choiceExplanations가 선지 내용과 무관한 문제** — 각 해설은 해당 선지가 왜 정답/오답인지를 직접 설명해야 하며, 선지와 다른 주제의 설명이면 탈락

✅ **포함 기준** (모두 충족해야 함):
- 정답의 근거가 학습자료에 명확히 존재
- 모든 오답 선지가 왜 틀린지 학습자료 기반으로 설명 가능
- choiceExplanations이 각 선지별로 정확한 근거 포함 — **각 해설이 해당 선지의 내용을 직접 언급**하는지 확인
...
- explanation은 정답에 대한 전체 해설입니다. **1~3문장으로 간결하게** 작성하세요. 사고 과정, 재검토, 자기 수정 등을 포함하지 마세요.
- choiceExplanations는 각 선지별로 왜 정답/오답인지 설명하는 배열입니다 (선지 순서대로, 필수). **각 선지 해설도 1~2문장으로 간결하게** 작성하세요.
  - ⚠️ **각 choiceExplanation은 반드시 해당 선지의 내용을 직접 언급하며 왜 옳은지/틀린지를 설명해야 합니다.** 선지와 무관한 다른 개념 설명을 넣으면 안 됩니다.
  - ❌ 금지: 선지는 "세포독성 T세포" 관련인데 해설이 "인터루킨(IL)은 ~이다" → 완전히 다른 주제
  - ✅ 올바른 예: 선지 "세포독성 T세포가 핵심 역할" → 해설 "이는 세포매개 면역의 특징이며, 체액성 면역과 다릅니다"
  - "다시 확인해보겠습니다", "재검토 후" 같은 사고 과정을 포함하지 마세요.
